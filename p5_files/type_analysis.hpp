...
public:
	...
	void errOutputCustom(const Position * pos){
		hasError = true;
		Report::fatal(pos,
			"Attempt to output a custom");
	}
	void errOutputCustomVar(const Position * pos){
		hasError = true;
		Report::fatal(pos,
	...
	void errReadCustom(const Position * pos){
		hasError = true;
		Report::fatal(pos,
			"Attempt to assign user input to class");
	}
	void errReadCustomName(const Position * pos){
		hasError = true;
		Report::fatal(pos,
			"Attempt to fromconsole a custom name");
	}
	void errReadCustomVar(const Position * pos){
		hasError = true;
		Report::fatal(pos,
	...
	void errEqCustomNames(const Position * pos){
		hasError = true;
		Report::fatal(pos,
			"Equality operator applied to custom names");
	}
	void errEqCustomVar(const Position * pos){
		hasError = true;
		Report::fatal(pos,
	...
	void errAssignCustomNames(const Position * pos){
		hasError = true;
		Report::fatal(pos,
			"custom name assignment");
	}
	void errAssignCustomVar(const Position * pos){
		hasError = true;
		Report::fatal(pos,
			"custom variable assignment");
	}

	void errAssignImm(const Position * pos){
		hasError = true;
		Report::fatal(pos,
			"Attempt to assign immutable");
	}
